...
public class roundTrip {
	...
	public static String verifiedRoundTrip(WebDriver driver, String from, String to) throws InterruptedException {
		...
		try {
			...
			WebElement tripType = driver.findElement(By
					.xpath("/html/body/div[1]/div/div[3]/div/section[1]/div/div/div[1]/div[2]/div[2]/div[1]/label[2]"));
			tripType.click();
...
//                  GetData DepartDate and Return date 
			...
			WebElement tripTypeListing = driver
					.findElement(By.xpath("/html/body/div[1]/div/div/div/div[1]/div/div[2]/div/div[3]/label/span"));
...
//          Compared Depart date widget and Listing page 
			...
			try {
				WebElement Show = wait.until(ExpectedConditions.visibilityOfElementLocated(
						By.xpath("/html/body/div[1]/div/div/div/section/div/div/div[1]/div/div/div/div/div[6]")));
				String modifiedString = Show.getText();
				String flightFilter = modifiedString.replaceFirst("Airlines\\n?", "");
...
//					}
//				}
					String[] words = flightFilter.split(" ");
					StringBuilder formattedString = new StringBuilder();

					for (int i = 0; i < words.length; i++) {
						formattedString.append(words[i]).append(" ");

						// Check if the next word is a number (price) and add a new line after it
						if (i + 1 < words.length) {
							try {
								Double.parseDouble(words[i + 1]); // Check if it's a price
					...
					availableAirlinesOnListing = formattedString.toString().trim();
					System.out.println(availableAirlinesOnListing);
				}
